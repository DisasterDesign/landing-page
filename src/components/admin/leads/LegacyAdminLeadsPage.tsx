...
import type {
  LeadDetail as CanonicalLeadDetail,
} from "@/lib/leads/projection";

import LeadMigrationReviewControls from "./LeadMigrationReviewControls";
import LeadOwnershipControls from "./LeadOwnershipControls";
...
export default function LegacyAdminLeadsPage() {
            ...
            activeFollowUp
              ? date
                ? {
                    ...
                    reason: "פולואפ שעודכן בממשק הישן",
                  }
                : {
                    action: "complete",
                    followUpId: activeFollowUp.id,
              ...
              : {
                  dueAt: new Date(date as string).toISOString(),
                  reason: "פולואפ שנקבע בממשק הישן",
                },
          ),
        });
        if (!res.ok) throw new Error();
  ...
  // Deep-link support: /admin/leads?focus=<id> fetches and expands the exact
  // row even when it falls outside the capped legacy list.
  useEffect(() => {
...
function ExpandedPanel({
  ...
  useEffect(() => {
    const handleLeadChanged = (event: Event) => {
      ...
      if (changedId && changedId !== leadId) return;
      void reload();
      onChange();
    };
    window.addEventListener("lead:changed", handleLeadChanged);
    return () => window.removeEventListener("lead:changed", handleLeadChanged);
  }, [leadId, onChange, reload]);
  ...
  if (detail.canonical.migrationReviewRequired) {
    return <LeadMigrationReviewControls lead={detail.canonical} />;
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div className="space-y-3">
        ...
        <LeadOwnershipControls lead={detail.canonical} />

        <ReasonedStageControls
          lead={detail.canonical}
          onChanged={async () => {
            await reload();
            onChange();
          }}
        ...
        {detail.canonical.capabilities.canCreateAgreement && (
          <a
            href={`/admin/agreements?new=1&leadId=${encodeURIComponent(leadId)}`}
            className="inline-flex rounded-full bg-green-500/20 px-3.5 py-1.5 text-xs font-bold text-green-300 hover:bg-green-500/30"
          >
            הכשר חוזה לליד
          </a>
        )}
...
type SafeStageAction = "mark-lost" | "mark-spam" | "reopen-lost";

function ReasonedStageControls({
  lead,
  onChanged,
}: {
  lead: CanonicalLeadDetail;
  onChanged: () => Promise<void>;
}) {
  const [action, setAction] = useState<SafeStageAction>(
    lead.stage === "LOST" ? "reopen-lost" : "mark-lost",
  );
  ...
  const availableActions = useMemo(() => {
    const values: Array<[SafeStageAction, string]> = [];
    if (lead.stage === "LOST") {
      values.push(["reopen-lost", "פתח מחדש"]);
    } else if (lead.stage !== "WON" && lead.stage !== "SPAM") {
      values.push(["mark-lost", "סיים כאבוד"]);
      if (lead.intentLevel !== "OUTBOUND") {
        values.push(["mark-spam", "סמן כספאם"]);
      }
    }
    return values;
  }, [lead.intentLevel, lead.stage]);

  useEffect(() => {
    if (
      availableActions.length > 0 &&
      !availableActions.some(([value]) => value === action)
    ) {
  ...
  async function submit() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(payload?.message ?? payload?.error ?? "עדכון הסטטוס נכשל");
      }
      toast.success("מסלול הליד עודכן ונשמר בהיסטוריה");
      setReason("");
      await onChanged();
    } catch (error) {
      toast.error(
  ...
  return (
    <section className="rounded-xl border border-gray-700 bg-gray-900 p-3">
      <p className="text-xs font-bold text-white">שינוי מסלול מתועד</p>
      <div className="mt-2 grid gap-2">
        <select
          value={action}
          onChange={(event) => setAction(event.target.value as SafeStageAction)}
          className="rounded-lg border border-gray-700 bg-gray-800 px-3 py-2 text-xs text-white outline-none focus:border-pink"
        >
        ...
        {action === "mark-lost" && (
          ...
          >
            ...
            <option value="BAD_TIMING">תזמון לא מתאים</option>
            <option value="EXISTING_PROVIDER">יש ספק קיים</option>
            <option value="DECISION_MAKER_UNREACHABLE">
              לא ניתן להגיע למקבל החלטות
            </option>
            <option value="NOT_FIT">לא מתאים לשירות</option>
            <option value="BAD_CONTACT">פרטי קשר שגויים</option>
            <option value="DUPLICATE">ליד כפול</option>
          </select>
        )}
        <input
          value={reason}
          onChange={(event) => setReason(event.target.value)}
          placeholder="סיבה לשינוי (נשמרת בהיסטוריה)"
          maxLength={500}
          className="rounded-lg border border-gray-700 bg-gray-800 px-3 py-2 text-xs text-white outline-none focus:border-pink"
        />
        <button
          ...
          onClick={() => void submit()}
          className="rounded-lg bg-pink/15 px-3 py-2 text-xs font-bold text-pink disabled:opacity-40"
        >
