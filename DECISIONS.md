...
## 2026-07-28 — מודל שותפים: שתי רמות חשיפה, חלוקה פר-שותף

**Status:** Accepted ומיושם (fde5baf + רוטציית סוד).

### Decision

שתי רמות: **בעלים** (אלעד, `User.isOwner`) רואה הכל כולל עסקאות פרטיות;
**שותף** (`role: SELLER`) רואה רק את מה שהוא ייצר, ואינו יודע על קיום
שותפים אחרים. אין `Role.PARTNER` — עמודת `isOwner` אורתוגונלית, כי enum
חדש היה שובר בשקט את ניתוב הלידים (`resolveEligibleSellerId` דורש SELLER).

תנאי חלוקה פר-שותף: `User.revenueSharePct` — מספר = אחוז מתמשך מהרווח על
עסקאות שהשותף ייצר (רועי: 50); null = עמלת חודש ראשון דרך `SellerCommission`
(דגרון, אלבז, דניאל בעתיד). שותף אחוזי לא מקבל עמלת חודש ראשון (כפל תשלום).

בעלות לקוח: `Client.ownerId`, מיוחס בכל נקודת יצירה (חתימה: credited →
creator → owner; יצירה ידנית; לקוח one-off). כל 30 הקיימים → רועי.
`Client.partner` נשאר תווית קוהורט נפרדת — לא בעלות.

הסינון בשאילתה, לא בתצוגה: `src/lib/auth/viewer.ts` — `requireOwner` על
61 מסלולי בעלים (קורא isOwner מה-DB, לא מה-JWT שמחזיק תפקיד 24 שעות),
scopes ללקוחות/הסכמים/לידים/משימות. רועי הורד ל-SELLER ב-28.7 +
רוטציית `NEXTAUTH_SECRET` לפסילת ה-JWT הישן.

### אימות

דוח השותפים החדש (pct) מול הישן (÷2): ₪1,825.79 בשניהם, זהה לשקל,
21 שורות, 0 לקוחות ללא בעלים. 353 טסטים ירוקים.

### Reversibility

`UPDATE "User" SET role='ADMIN'` מחזיר את רועי מיידית. העמודות אדיטיביות.
